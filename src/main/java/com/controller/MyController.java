	...
	//@Qualifier("farmerService")
	 ...
	 @RequestMapping(value = "/Farmerregister", method = RequestMethod.GET)
	  ...
	  public ModelAndView loginProcess(HttpServletRequest request, HttpServletResponse response,   @ModelAttribute Farmer ifarmer) {
	    ...
	    if (flag) {
	      mav = new ModelAndView("Farmer_home");
	      mav.addObject("full_name", ifarmer.getF_email());
	      //session manage
	      
	      HttpSession session= request.getSession();
	      session.setAttribute("email", ifarmer.getF_email());
	    } else {
	      mav = new ModelAndView("loginf");
	      mav.addObject("message", "farmeremail or Password is wrong!! or you are not a registered user");
	    }
	    return mav;
	  }

@RequestMapping(value = "/logoutf", method = RequestMethod.GET)
		 public ModelAndView farmerlogout2(HttpServletRequest request, HttpServletResponse response) {
